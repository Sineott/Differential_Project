...
for i = 1:size(x_true, 1)
    ...
    filename = fullfile(output_dir, ['UKF_', variable_names{i}, '.png']);
    saveas(fig, filename);
    fprintf('Saved figure: %s\n', filename);
    
    % Also save as PDF for high quality
    filename_pdf = fullfile(output_dir, ['UKF_', variable_names{i}, '.pdf']);
...
fprintf('RMSE Values:\n');
for i = 1:size(x_true, 1)
    fprintf('%s: %.4f\n', display_names{i}, rmse_values(i));
end

% Save RMSE values to a CSV file
rmse_table = table(display_names', rmse_values, 'VariableNames', {'Variable', 'RMSE'});
writetable(rmse_table, fullfile(output_dir, 'UKF_RMSE_Values.csv'));
fprintf('Saved RMSE values to: %s\n', fullfile(output_dir, 'UKF_RMSE_Values.csv'));
...
filename_combined = fullfile(output_dir, 'UKF_All_Variables_Combined.png');
saveas(fig_combined, filename_combined);
fprintf('Saved combined figure: %s\n', filename_combined);

% Also save as PDF for high quality
filename_combined_pdf = fullfile(output_dir, 'UKF_All_Variables_Combined.pdf');
print(fig_combined, filename_combined_pdf, '-dpdf', '-r300');
fprintf('Saved combined figure as PDF: %s\n', filename_combined_pdf);

% Create timestamp file to indicate when the analysis was completed
timestamp = datestr(now, 'yyyy-mm-dd_HH-MM-SS');
fid = fopen(fullfile(output_dir, ['analysis_completed_', timestamp, '.txt']), 'w');
fprintf(fid, 'UKF Economic Analysis completed at: %s\n', datestr(now));
fclose(fid);

disp(['All figures have been saved to the ', output_dir, ' directory.']);