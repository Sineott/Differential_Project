...
% Create individual plots with comet animation
for i = 1:size(x_true, 1)
    ...
    % Run comet animation for the estimate
    comet(time, x_est(i,:), 0.);
...
% Create a combined figure with normalized data and comet animation
...
for i = 1:size(x_true, 1)
    ...
    scatter(time, (z_meas(i,:) - min(x_true(i,:))) / (max(x_true(i,:)) - min(x_true(i,:))), 10, 'g', 'filled', 'MarkerFaceAlpha', 0.3);
...
% Run comet animation for each subplot
for i = 1:size(x_true, 1)
    subplot(2, 4, i);
    comet(time, norm_est(i,:), 0.1);
end