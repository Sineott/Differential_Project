...
% Variable names for plots (use file-safe names without slashes)
variable_names = {'GDP_Output', 'Labour', 'Capital', 'Depreciation', 'Savings_Rate', 'Productivity', 'Labour_Growth_Rate'};
display_names = {'GDP/Output', 'Labour', 'Capital', 'Depreciation', 'Savings Rate', 'Productivity', 'Labour Growth Rate'};

% Set video parameters
fps = 15;           % Frames per second
frames_per_step = 2; % Number of frames for each time step
total_frames = N * frames_per_step;

% Create a folder for videos if it doesn't exist
if ~exist('ekf_videos', 'dir')
    mkdir('ekf_videos');
end
...
for i = 1:size(x_true, 1)
    % Create figure
    fig = figure('Name', display_names{i}, 'Position', [100*i, 100*i, 800, 400]);
    
    % Create safe filename (replace any potentially problematic characters)
    safe_filename = variable_names{i};
    
    % Initialize VideoWriter
    video_filename = fullfile('ekf_videos', [safe_filename, '_animation.mp4']);
    ...
    % Initialize estimated value line (will be updated frame by frame)
    h_est = plot(NaN, NaN, 'r-', 'LineWidth', 2);
    
    % Set up plot properties
    title([display_names{i}, ': Actual vs Estimated']);
    xlabel('Time Step');
    ylabel('Value');
    ...
    % Set axis limits
    ylim([min(x_true(i,:))*0.9, max(x_true(i,:))*1.1]);
    
    % Create animation frames
    for frame = 1:total_frames
        % Calculate current time step based on frame number
        current_step = min(ceil(frame/frames_per_step), N);
        
        % Update the estimate line to show progress
        set(h_est, 'XData', time(1:current_step), 'YData', x_est(i,1:current_step));
        
        % Add a marker at the current point
        if current_step > 1
            hold on;
            scatter(time(current_step), x_est(i,current_step), 50, 'r', 'filled');
            hold off;
        end
        ...
        % Remove the marker for next iteration
        if current_step > 1
    ...
    close(v);
    fprintf('Saved video for %s to: %s\n', display_names{i}, video_filename);
    
    % Close the figure
    close(fig);
end

% Create a combined animation for all normalized variables
...
% Initialize VideoWriter for combined animation
video_filename = fullfile('ekf_videos', 'All_Variables_Combined.mp4');
v_combined = VideoWriter(video_filename, 'MPEG-4');
v_combined.FrameRate = fps;
open(v_combined);

% Create 2x4 subplot grid for all variables
subplot_h = cell(1, size(x_true, 1));
for i = 1:size(x_true, 1)
    subplot(2, 4, i);
    ...
    % Initialize estimated value line (will be updated frame by frame)
    subplot_h{i} = plot(NaN, NaN, 'r-', 'LineWidth', 1.5);
...
% Create animation frames for combined plot
for frame = 1:total_frames
    % Calculate current time step based on frame number
    current_step = min(ceil(frame/frames_per_step), N);
    ...
    for i = 1:size(x_true, 1)
        % Update the estimate line to show progress
        set(subplot_h{i}, 'XData', time(1:current_step), 'YData', norm_est(i,1:current_step));
        
        % Add a marker at the current point in each subplot
        subplot(2, 4, i);
        if current_step > 1
            hold on;
            scatter(time(current_step), norm_est(i,current_step), 30, 'r', 'filled');
            hold off;
        end
    end
    ...
    % Capture the frame
    drawnow;
    frame_data = getframe(fig_combined);
    writeVideo(v_combined, frame_data);
    
    % Remove the markers for next iteration
    for i = 1:size(x_true, 1)
        subplot(2, 4, i);
        delete(findobj(gca, 'Type', 'scatter'));
    end
...
% Close the video file
close(v_combined);
fprintf('Saved combined video to: %s\n', video_filename);

% Close the figure
close(fig_combined);

% Calculate overall RMSE for each variable (no display on graph)
rmse_values = zeros(size(x_true, 1), 1);
for i = 1:size(x_true, 1)
    rmse_values(i) = sqrt(mean((x_true(i,:) - x_est(i,:)).^2));
end

fprintf('\nAll videos saved in the "ekf_videos" folder.\n');